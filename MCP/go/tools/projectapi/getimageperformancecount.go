...
func GetimageperformancecountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/iterations/%s/performance/images/count%s", cfg.BaseURL, projectId, iterationId, queryString)
...
func CreateGetimageperformancecountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_iterations_iterationId_performance_images_count",
		mcp.WithDescription("Gets the number of images tagged with the provided {tagIds} that have prediction results from
training for the provided iteration {iterationId}"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetimageperformancecountHandler(cfg),
	}
}
