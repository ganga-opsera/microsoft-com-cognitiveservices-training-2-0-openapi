...
func GetiterationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetiterationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_iterations_iterationId",
		mcp.WithDescription("Get a specific iteration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetiterationHandler(cfg),
	}
}
