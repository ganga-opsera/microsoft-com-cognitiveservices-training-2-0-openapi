...
func ExportiterationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["flavor"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("flavor=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/projects/%s/iterations/%s/export%s", cfg.BaseURL, projectId, iterationId, queryString)
...
func CreateExportiterationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_projects_projectId_iterations_iterationId_export",
		mcp.WithDescription("Export a trained iteration"),
		...
		mcp.WithString("platform", mcp.Required(), mcp.Description("The target platform (coreml or tensorflow)")),
		mcp.WithString("flavor", mcp.Description("The flavor of the target platform (Windows, Linux, ARM, or GPU)")),
		mcp.WithString("Training-Key", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ExportiterationHandler(cfg),
	}
}
