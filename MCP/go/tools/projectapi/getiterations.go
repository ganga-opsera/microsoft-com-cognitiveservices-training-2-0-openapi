...
func GetiterationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/iterations", cfg.BaseURL, projectId)
...
func CreateGetiterationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_iterations",
		mcp.WithDescription("Get iterations for the project"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetiterationsHandler(cfg),
	}
}
