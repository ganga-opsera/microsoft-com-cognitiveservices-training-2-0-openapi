...
func GetexportsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/iterations/%s/export", cfg.BaseURL, projectId, iterationId)
...
func CreateGetexportsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_iterations_iterationId_export",
		mcp.WithDescription("Get the list of exports for a specific iteration"),
