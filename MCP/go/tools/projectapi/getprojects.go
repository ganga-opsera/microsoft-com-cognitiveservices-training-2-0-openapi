...
func GetprojectsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetprojectsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects",
		mcp.WithDescription("Get your projects"),
		mcp.WithString("Training-Key", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetprojectsHandler(cfg),
	}
}
