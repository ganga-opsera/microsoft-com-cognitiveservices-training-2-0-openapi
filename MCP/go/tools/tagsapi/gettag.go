...
func GettagHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/tags/%s%s", cfg.BaseURL, projectId, tagId, queryString)
...
func CreateGettagTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_tags_tagId",
		mcp.WithDescription("Get information about a specific tag"),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("The project this tag belongs to")),
		mcp.WithString("tagId", mcp.Required(), mcp.Description("The tag id")),
		mcp.WithString("iterationId", mcp.Description("The iteration to retrieve this tag from. Optional, defaults to current training set")),
		mcp.WithString("Training-Key", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GettagHandler(cfg),
	}
}
