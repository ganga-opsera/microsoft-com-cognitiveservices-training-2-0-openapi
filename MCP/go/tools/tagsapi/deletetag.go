...
func DeletetagHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletetagTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_projects_projectId_tags_tagId",
		mcp.WithDescription("Delete a tag from the project"),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("The project id")),
		mcp.WithString("tagId", mcp.Required(), mcp.Description("Id of the tag to be deleted")),
		mcp.WithString("Training-Key", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletetagHandler(cfg),
	}
}
