...
func DeleteimagetagsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/images/tags%s", cfg.BaseURL, projectId, queryString)
...
func CreateDeleteimagetagsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_projects_projectId_images_tags",
		mcp.WithDescription("Remove a set of tags from a set of images"),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("The project id")),
		mcp.WithArray("imageIds", mcp.Required(), mcp.Description("Image ids. Limited to 64 images")),
		mcp.WithArray("tagIds", mcp.Required(), mcp.Description("Tags to be deleted from the specified images. Limited to 20 tags")),
		mcp.WithString("Training-Key", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteimagetagsHandler(cfg),
	}
}
