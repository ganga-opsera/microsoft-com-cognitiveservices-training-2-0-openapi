...
func CreateimagetagsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ImageTagCreateBatch
		...
		url := fmt.Sprintf("%s/projects/%s/images/tags", cfg.BaseURL, projectId)
		...
		var result models.ImageTagCreateSummary
...
func CreateCreateimagetagsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_projects_projectId_images_tags",
		mcp.WithDescription("Associate a set of images with a set of tags"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CreateimagetagsHandler(cfg),
	}
}
