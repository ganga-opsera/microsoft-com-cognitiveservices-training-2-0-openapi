...
func GetuntaggedimagecountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/images/untagged/count%s", cfg.BaseURL, projectId, queryString)
...
func CreateGetuntaggedimagecountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_images_untagged_count",
		mcp.WithDescription("Gets the number of untagged images"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetuntaggedimagecountHandler(cfg),
	}
}
