...
func GetuntaggedimagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/images/untagged%s", cfg.BaseURL, projectId, queryString)
...
func CreateGetuntaggedimagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_images_untagged",
		mcp.WithDescription("Get untagged images for a given project iteration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetuntaggedimagesHandler(cfg),
	}
}
