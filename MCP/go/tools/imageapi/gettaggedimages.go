...
func GettaggedimagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/images/tagged%s", cfg.BaseURL, projectId, queryString)
...
func CreateGettaggedimagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_images_tagged",
		mcp.WithDescription("Get tagged images for a given project iteration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GettaggedimagesHandler(cfg),
	}
}
